...
function getVendor(owner: Package, resolver: Resolver, filename: string): string {
  ...
  let path = join(locateEmbroiderWorkingDir(resolver.options.appRoot), 'environment.json');
  if (!lstatSync(path).isFile()) {
    throw new Error(`Failed to read the environment.json when generating content for ${filename}`);
  }
  let emberENV = readJSONSync(path).EmberENV;
